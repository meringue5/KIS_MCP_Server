...
연결 전략:
  - KIS_DB_MODE=motherduck (기본): MotherDuck (md:kis_portfolio)
  - KIS_DB_MODE=local: 로컬 DuckDB (var/local/kis_portfolio.duckdb)
...
import duckdb
from .config import get_db_mode, get_local_db_path, get_motherduck_database, get_motherduck_token
        ...
        try:
            ...
            if adjusted:
                ...
                """, [r["symbol"], r["exchange"], d,
                      ...
                      _f(r.get("low")), _f(r.get("close")), _i(r.get("volume"))]).fetchone()
                saved += 1
            else:
...
def get_price_history(symbol: str, exchange: str,
    ...
    return [_normalize_row(dict(zip(cols, r))) for r in rows]


def has_price_history(symbol: str, exchange: str,
                      start_date: str, end_date: str) -> bool:
    """
    해당 기간에 캐시 데이터가 있는지 확인.

    거래일 캘린더가 없으므로 "완전성"은 판단하지 않고, 요청 구간 안에
    최소 1개 이상 저장된 가격 행이 있는지만 확인한다.
    """
    con = get_connection()
    s = datetime.strptime(start_date, "%Y%m%d").date()
    e = datetime.strptime(end_date, "%Y%m%d").date()
    count = con.execute("""
        ...
        FROM price_history
        WHERE symbol=? AND exchange=? AND date BETWEEN ? AND ?
    """, [symbol, exchange, s, e]).fetchone()[0]
    return count > 0
    ...
    for r in rows:
        try:
            ...
            inserted = con.execute("""
                ...
                VALUES (?, ?, ?, ?)
                RETURNING 1
            """, [currency.upper(), d, period, _f(r.get("rate"))]).fetchone()
            if inserted:
                saved += 1
        except Exception as e:
...
def _normalize_row(row: dict) -> dict:
    """DuckDB 결과를 MCP JSON 응답에 안전한 값으로 변환."""
    normalized = {}
    for key, value in row.items():
        if isinstance(value, (datetime, date)):
            normalized[key] = value.isoformat()
        elif key.endswith("_data") or key == "data":
            normalized[key] = _json_loads(value)
        else:
