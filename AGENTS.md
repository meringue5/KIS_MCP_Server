...
├── tests/             # pytest 테스트 위치
├── docs/              # 세부 운영/설계 문서
│   └── security-and-secrets.md # 시크릿/source of truth/토큰 저장 정책
...
### Secret / token 관리 문서
시크릿과 토큰의 canonical policy는 `docs/security-and-secrets.md`에 둔다.
`SPEC.md`는 설계 결정, `ARCHITECTURE.md`는 trust boundary, `docs/deployment.md`는 배포 절차만 담당한다.
