...
토큰 파일은 `issued_at`, `expires_at`, KIS 응답 만료 메타데이터를 저장하며,
만료 10분 전부터 새 발급 대상으로 본다. 같은 계좌 토큰 refresh는 파일락으로 직렬화한다.
...
### Remote MCP 인증
원격 Streamable HTTP MCP는 `kis-mcp-remote` 엔트리포인트로 실행한다.
`/mcp` endpoint는 `Authorization: Bearer <KIS_REMOTE_AUTH_TOKEN>` 헤더를 요구한다.
`/healthz`만 인증 없이 노출한다. 운영 환경에서 `KIS_REMOTE_AUTH_DISABLED=true`를 사용하지 않는다.
...
`src/kis_mcp_server/db/` 패키지가 DB 레이어를 담당. `src/kis_mcp_server/app.py`는
`from . import db as kisdb`로 사용. 루트에는 MCP 설정 호환을 위한 `server.py`만 남긴다.
